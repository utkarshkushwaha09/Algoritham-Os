...
const PriorityNonPreemptive = ({ processes }) => {
  ...
  useEffect(() => {
    ...
    newInstr.push(
      <span key="start">
        Sort all processes by <strong>arrival time</strong>. Then, at each decision point, choose the process with the <strong>highest priority</strong> (lowest priority number) among those that have arrived and are not yet executed.
      </span>
    );
    ...
    ganttChart.forEach((p, i) => {
      ...
      newInstr.push(
        <span key={`exec-${i}`}>
          Process <strong style={{ color: "green" }}>{p.name}</strong> (Priority = {p.priority}) is selected and executed from t={p.startTime} to t={p.endTime}.
        </span>
      );
    });
    ...
    newInstr.push(
      <span key="end">
        Finally, construct the Gantt chart based on the execution order of selected highest-priority processes.
      </span>
    );
  ...
  return (
    <div className="flex">
      ...
      {/* <div className="flex flex-col items-center p-6 bg-purple-400/50 rounded-lg border border-white text-white min-w-[80vw] mx-auto"> */}
          ...
          <div className="w-full bg-purple-400/50 p-4 rounded-lg border border-white">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {pendingProcesses.map((p) => (
                <div
                  key={p.name}
                  className="p-4 bg-yellow-300 rounded-lg border border-white text-center w-32 transition-all duration-500"
                  style={{
                ...
                >
                  ...
                  <div className="grid grid-cols-2  gap-1 mt-2 text-sm">
                    <p className="bg-sky-500 rounded p-1">Arrival: {p.arrivalTime}</p>
                    <p className="bg-sky-500 rounded p-1">Burst: {p.burstTime}</p>
                    <p className="bg-sky-500 rounded p-1 col-span-2">Priority: {p.priority}</p>
                  </div>
                </div>
              ))}
          ...
          <div className="w-full bg-purple-400/50 p-4 rounded-lg border border-white">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {arrivedQueue.map((p) => (
                <div
                  key={p.name}
                  className={`p-4 rounded-lg border bg-orange-300 text-center transition-all duration-500 w-32 ${
                    fadeOutProcess === p.name
                      ? "opacity-0 scale-75  transform  bg-orange-500 translate-y-4"
                      : currentProcess && currentProcess.name === p.name
                        ? "scale-110 border-yellow-500 border-2 bg-orange-400 bg-opacity-30"
                        : comparingProcess && comparingProcess.name === p.name
                          ? "scale-105 border-red-500 border-2 bg-amber-600 bg-opacity-30"
                          : highlightNewArrivals.includes(p.name)
                            ? "scale-110 border-green-500 border-2 bg-amber-400 animate-pulse"
                            : "border-white"
                  }`}
                  style={{
                    borderLeft: `5px solid ${processColors[p.name] || "#3498db"}`,
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-1 mt-2 text-sm">
                    ...
                    <p className="bg-sky-400 rounded p-1">Burst: {p.burstTime}</p>
                    <p className="bg-sky-400 rounded p-1 col-span-2">Priority: {p.priority}</p>
                  </div>
                </div>
              ))}
